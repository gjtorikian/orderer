...
| `TWILIO_PHONE` | Yes | Twilio sender number. Format: `+1234567890`. |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `WARMUP_TRADES` | `2` | Number of initial trades to use as a regime signal. If all warmup trades win, hot mode activates. |
...
3. If all warmup trades win consecutively, **hot mode** activates: `maxSpend` is multiplied by `HOT_MULTIPLIER` for all remaining trades that day.
4. If warmup trades include `COLD_SHUTDOWN_LOSSES` or more losses, the bot enters **cold shutdown** and rejects all further signals for the day.
...
## Signal Classification (SLOTS mode)

When the POST to `/place` includes a `metrics` object, the bot classifies the signal before trading:

| Direction | Filter | Description |
|-----------|--------|-------------|
| **Long** (buy) | `onBalanceRun > 1.9724 AND vwapGain > 3.4966` | Moderate tier — reliable directional signal with positive expected value. |
| **Short** (sell short) | `boxRatio < 0.1939 AND thrust > 5.2361` | Reversal signal — stock predicted to drop. 98.8% historical short-side WR. |
| **Skip** | Neither filter matches, or both match (conflict) | Trade is rejected with 204. |

If no `metrics` are provided in the POST body, the bot falls back to the legacy behavior (treats all `b ` messages as long buys).
